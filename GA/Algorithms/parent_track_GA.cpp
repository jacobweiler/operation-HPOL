...
*/
void dataWrite(int numChildren, vector<vector<vector<float>>> &varVector, int freq_coeffs, vector<double> freqVector, vector<int> selected);
...
void crossover(vector<vector<vector<float> > > & varInput, vector<vector<vector<float> > > & varOutput, vector<float> fitness, vector<int> P_loc, vector<int> & selected, float roul_percentage, float tour_percentage, float rank_percentage, int crossover_no, int pool_size, int reproduction_no);
	  ...
	  {cout << "Error: Usage. Specify start or cont, as well as NPOP (EX: start 10)." << endl << "Form: start/cont NPOP repro_no cross_no roul_no tour_no rank_no elite:1/0" << endl;} 
      		...
      		if(string(argv[1]) == "start")
		{
		  // seession with kai (Debolt 10/27/2020) normal dist instead of uniform???
			...
			vector<int> selected;
			dataWrite(NPOP, varOutput, freq_coeffs, freqVector, selected);
		...
		else if(string(argv[1]) == "cont")
		{
			  ...
			  crossover(varInput, varOutput, fitness, P_loc, selected, ROULETTE_PROPORTION, TOURNEY_PROPORTION, RANK_PROPORTION, crossover_no, pool_size, reproduction_no);
