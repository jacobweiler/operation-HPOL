...
void trial_mutation(vector<vector<vector<float> > > & varInput, vector<vector<vector<float> > > & varOutput, int reproduction_no, int crossover_no, float tour_percentage, float roul_percentage, int pool_size, vector<float> fitness);
...
void roulette(vector<vector<vector<float>>> &varInput, vector<vector<vector<float>>> &varOutput, vector<float> &fitness, default_random_engine& random_sequence)
{
	...
	*/
	...
	int roulette_no= 0; //NPOP*(1-TOURNEY_PROPORTION);
	int tourney_no= 1; //NPOP*TOURNEY_PROPORTION;
...
void tournament(vector<vector<vector<float>>> &varInput, vector<vector<vector<float>>> &varOutput, vector<float> &fitness, default_random_engine& random_sequence)
{
  ...
  int roulette_no= 1; //NPOP*(1-TOURNEY_PROPORTION);
  int tourney_no = 0; //=NPOP*TOURNEY_PROPORTION;
...
for(int r=0; r<roul_no; r++)
{
 ...
 for(int t=roul_no; t<crossover_no; t++) //changed to start at roul and go to crossover to stop seg fault 11/23/2020 Debolt//
{
...
void trial_mutation(vector<vector<vector<float> > > & varInput, vector<vector<vector<float> > > & varOutput, int reproduction_no, int crossover_no, float tour_percentage, float roul_percentage, int pool_size, vector<float> fitness)
{
  ...
  int tour_no =  tour_percentage*(varOutput.size()-(reproduction_no+crossover_no));
  int location = 0;

  for(int i=reproduction_no+crossover_no; i<varOutput.size(); i++)
    {
      if(i<=tour_no+reproduction_no+crossover_no)
	{
	  int location = new_tournament(fitness, pool_size);
	}
      else if(i>tour_no+crossover_no+reproduction_no)
	{
	  location = new_roulette(fitness);
	}
      int mut_section = rand() % 1;
      int mut_gene = rand() % 2;
      ...
      for(int j=0; j<NSECTIONS; j++)
	{
	  for(int k=0; k<NVARS; k++)
	      ...
	      if(j==mut_section)
		{
		  if(mut_gene==0 && k==0)
		    {
		      varOutput[i][j][k] = r_mut(generator);
		    }
		  else if(mut_gene == 1 && k==1)
		    {
		      varOutput[i][j][k] = l_mut(generator);
		    }
		  else if(mut_gene == 2 && k==2)
		    {
		      varOutput[i][j][k] = a_mut(generator);
		    }
		}
	      else
		{
		  varOutput[i][j][k] = varInput[location][j][k];
		}
	    }
	  if(varOutput[i][j][0]  + (varOutput[i][j][1]* tan(varOutput[i][j][2])) > max_outer_radius)
	    {
