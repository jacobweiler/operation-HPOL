...
const float TOURNEY_PROPORTION = 0.5f; // This value represents what percentage of the children generated use tournament-style selection. Must be 0 < x < 1.
...
const float reproduction_percent = .1;
const float crossover_percent = .75;
			...
			*/
			...
			for(int i=0;i<NPOP;i++)
			{
				for(int j=0;j<NSECTIONS;j++)
				{
					...
					if(varOutput[i][j][0]+varOutput[i][j][1]*tan(varOutput[i][j][2]) >= limit)
					  {
					    j = j-1;
					  }
				}
