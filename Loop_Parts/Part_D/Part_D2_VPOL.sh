	...
	for file in *; do
		if [ "$file" != "*" ] && [ "$file" != "" ]; then
			...
			if 	grep "segmentation violation" $RunDir/AraSim_Errors/${current_file}.error || grep "DATA_LIKE_OUTPUT" $RunDir/AraSim_Errors/${current_file}.error || 
				...
				sbatch --export=ALL,gen=$gen,num=${current_individual},WorkingDir=$WorkingDir,RunName=$RunName,Seeds=${current_seed},AraSimDir=$AraSimExec,nnt_per_ara=$nnt_per_ara --job-name=AraSimCall_AraSeed_$gen_${current_individual}_${current_seed}.run --output=$output_name --error=$error_name Batch_Jobs/AraSimCall_ParallelArray.sh
